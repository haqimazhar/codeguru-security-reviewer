...
;// CONCATENATED MODULE: ./src/utils/githubClient.ts
// githubClient.ts
...
class GitHubReviewClient {
    ...
    prNumber;
    commitSha;
    constructor(token, repository, prNumber, commitSha) {
    ...
    async createReviewComment(comment) {
        try {
            ...
            console.log(`Review comment created successfully for ${comment.path} at line ${comment.line}`);
        }
        catch (error) {
            console.error(`Error creating review comment for ${comment.path} at line ${comment.line}:`, error);
            throw error;
        }
    }
}
...
;// CONCATENATED MODULE: ./src/utils/sarifParser.ts
// sarifParser.ts
...
class SarifParser {
    ...
    constructor(sarifFilePath) {
        this.sarifPath = external_path_default().join(process.cwd(), sarifFilePath);
    }
    parse() {
        try {
            const sarifContent = external_fs_default().readFileSync(this.sarifPath, 'utf8');
...
const validateEnvironment = () => {
    ...
    return {
        ...
        INPUT_SARIF_PATH: process.env.INPUT_SARIF_PATH || 'codeguru-security-results.sarif.json',
...
const createReview = async () => {
    try {
        ...
        const githubClient = new GitHubReviewClient(env.INPUT_TOKEN, env.INPUT_REPOSITORY, env.INPUT_PULL_REQUEST_NUMBER, env.INPUT_INITIAL_COMMIT_SHA);
        ...
        console.log(`Existing comments: ${JSON.stringify(existingComments, null, 2)}`);
...
var __webpack_exports__createReview = __webpack_exports__.b;
export { __webpack_exports__createReview as createReview };
